  ...
  return (
    <header className="sticky top-0 z-50 grid h-[62px] flex-[0_0_62px] grid-cols-[1fr_auto_1fr] items-center overflow-hidden border-b border-subtle bg-[rgba(8,9,15,0.72)] px-[26px] backdrop-blur-[14px] max-bp768:grid-cols-[auto_1fr_auto] max-bp768:gap-1.5 max-bp768:px-4 max-bp600:px-3 max-bp480:h-[52px] max-bp380:px-2 light:bg-[rgba(236,234,244,0.86)]">
      <div className="flex flex-col leading-none">
        <span className="font-serif text-[22px] tracking-[.4px] text-gold [text-shadow:0_0_22px_var(--accent-glow)] max-bp768:text-[17px] max-bp600:text-[15px] max-bp480:text-[14px] max-bp380:text-[13px]">{wordmark}</span>
      ...
      <div className="flex min-w-0 items-center justify-end gap-2.5 overflow-visible">
              ...
              {currencies.map((c) => (
                <button
                  ...
                  className={
                    ...
                    (c === baseCurrency ? "bg-wash text-gold" : "text-secondary hover:bg-elevated hover:text-primary")
                  }
                  onClick={() => switchCurrency(c)}
                >
                  <span className="min-w-7 font-mono text-[13px] font-bold tabular-nums">{CCY_SYMBOL[c] ?? c}</span>
        ...
        <div className="flex items-center gap-2 max-bp600:hidden">
          <button
            ...
            disabled={spin}
          >
            <Icon name="refresh" size={16} className={spin ? "animate-spin-once" : ""} />
          </button>
          {onTweaksToggle && (
            <button className={ICON_BTN} onClick={onTweaksToggle} title="Tweaks">
