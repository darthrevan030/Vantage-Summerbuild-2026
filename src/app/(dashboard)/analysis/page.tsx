...
const labelFor = (s: number) =>
  s >= 25 ? "Bullish" : s <= -25 ? "Bearish" : "Neutral";
const clsFor = (s: number) => (s >= 25 ? "bull" : s <= -25 ? "bear" : "neut");
const toneFor = (s: number) =>
  s >= 25 ? "var(--gain)" : s <= -25 ? "var(--loss)" : "var(--gold)";
...
const FALLBACK_ITEMS: Record<
  string,
  { score: number; summary: string; drivers: string[] }
> = {
  AAPL: {
    score: 62,
    summary:
      "Resilient iPhone demand and double-digit services growth; the AI roadmap and steady buybacks support the multiple.",
    drivers: ["Services margin", "AI roadmap", "Buybacks"],
  },
  CICT: {
    score: 16,
    summary:
      "Singapore retail and office occupancy stays firm; rate cuts would help, but new office supply caps the upside.",
    drivers: ["Rate cuts", "High occupancy", "Office supply"],
  },
  IWDA: {
    score: 47,
    summary:
      "Broad developed-market exposure rides the global rally, though heavy US mega-cap weighting concentrates the risk.",
    drivers: ["Global rally", "US mega-caps", "Low cost"],
  },
  ASML: {
    score: 58,
    summary:
      "Semicon equipment demand driven by AI investment cycle; US export controls are the main risk.",
    drivers: ["AI capex", "Export controls", "Monopoly"],
  },
  FMG: {
    score: 22,
    summary:
      "Iron ore demand uncertain as China stimulus expectations fade; strong yield and low debt partially offset.",
    drivers: ["China demand", "Dividend", "Ore price"],
  },
};
const FALLBACK_OVERALL = {
  score: 0,
  note: "AI Analysis Currently Unavailable",
};
...
function sentPath(id: string, score: number) {
  ...
  const n = 12,
    start = score - 24 + rnd() * 16;
  const pts: number[] = [];
  for (let i = 0; i < n; i++) {
    const t = i / (n - 1);
    const base = start * (1 - t) + score * t;
    pts.push(
      Math.max(-100, Math.min(100, Math.round(base + (rnd() - 0.5) * 20))),
    );
  }
  pts[n - 1] = score;
  return pts;
...
function MiniSpark({
  ...
  const line = pts
    .map(
      (v, i) => `${i ? "L" : "M"}${(i * step).toFixed(1)},${Y(v).toFixed(1)}`,
    )
    .join(" ");
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="relative flex-1 h-2.5 rounded-md bg-elevated">
        ...
        <div
          className="absolute top-0 bottom-0 rounded-md [transition:width_.6s_cubic-bezier(.2,.7,.2,1),left_.6s_cubic-bezier(.2,.7,.2,1)]"
          style={{
            left: left + "%",
            width: w + "%",
      ...
      <span
        className="font-mono text-[13px] font-semibold min-w-[38px] text-right"
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const items: HlItem[] = Array.isArray(data)
          ? data.filter((n: HlItem) => n.t)
          : [];
        HL_CACHE[id] = items;
      ...
      <div className="flex flex-col gap-[5px] bg-elevated border border-subtle rounded-[11px] px-[13px] py-[11px]">
        <div className="flex items-center justify-between">
          <span className="font-ui text-secondary text-[11px] tracking-[.04em]">
            {useRealPrices ? "30-day price" : "30-day sentiment"}
          </span>
          <span
            className="font-mono text-[11px] tracking-[.04em]"
            style={{ color: delta >= 0 ? "var(--gain)" : "var(--loss)" }}
          >
          ...
          [0, 1, 2].map((i) => (
            ...
            >
              ...
              >
                <div
                  className="bg-elevated rounded-lg animate-skeleton"
                  style={{ height: 11, width: "88%" }}
                />
                <div
                  className="bg-elevated rounded-lg animate-skeleton"
                  style={{ height: 9, width: 70 }}
                />
              </div>
            </div>
          ))}
        {hl === "no-key" && (
          <div className="text-xs text-muted py-2.5 font-ui">
            No{" "}
            <code className="font-mono text-[11px] text-secondary">
              FINNHUB_API_KEY
            </code>{" "}
            — set it to load live headlines for {name}.
          </div>
        )}
        {hl === "empty" && (
          <div className="text-xs text-muted py-2.5 font-ui">
            No recent headlines found for {name}.
          </div>
        )}
        {Array.isArray(hl) &&
          hl.map((h, i) => (
  ...
  return (
    ...
    >
      <div className="flex items-center gap-[11px]">
        <div className="w-7 h-7 rounded-[7px] bg-elevated border border-subtle grid place-items-center text-gold flex-[0_0_auto]">
          <Icon name={it.icon as never} size={15} />
        </div>
        <div className="flex flex-col gap-px min-w-0 flex-1">
          <span className="font-ui text-[14px] font-semibold text-primary whitespace-nowrap overflow-hidden text-ellipsis">
            {it.name}
          </span>
          <span className="font-mono text-[10.5px] text-muted tracking-[.05em]">
            {it.ticker.replace(/_\d+$/, "")}
          </span>
        </div>
        <span
          className={
            "font-ui text-[11px] font-semibold px-[11px] py-1 rounded-full whitespace-nowrap tracking-[.02em] " +
            (PILL_CLS[clsFor(it.score)] ?? "")
          }
        >
          {labelFor(it.score)}
        </span>
      ...
      <div className="text-[12.8px] leading-[1.55] text-secondary">
        {it.summary}
      </div>
      {it.drivers.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {it.drivers.map((d, i) => (
            <span
              className="font-ui text-[11px] text-secondary bg-elevated border border-subtle rounded-[7px] px-[9px] py-1"
              key={i}
            >
              {d}
            </span>
      ...
      <button
        className="mt-px flex items-center justify-center gap-1.5 w-full bg-transparent border-t border-subtle pt-[11px] pb-px px-0 text-secondary font-ui text-xs cursor-pointer transition-colors duration-150 hover:text-gold"
        onClick={() => setOpen((o) => !o)}
      >
        ...
        <Icon
          ...
          className={
            open
              ? "rotate-180 transition-transform duration-[250ms]"
...
function Hero({
  ...
  items.forEach((i) => {
    counts[clsFor(i.score) as "bull" | "neut" | "bear"]++;
  });
  ...
  return (
    <div className="card relative grid grid-cols-[minmax(180px,250px)_1fr] gap-[34px] items-center overflow-hidden px-5 py-4.5 max-bp768:overflow-hidden max-bp480:p-3.5 max-bp380:p-3 max-bp1080:grid-cols-[1fr] max-bp1080:gap-[22px] before:content-[''] before:absolute before:inset-0 before:bg-[radial-gradient(70%_130%_at_14%_18%,var(--accent-glow)_0%,transparent_58%)] before:opacity-60 before:pointer-events-none animate-reveal">
      <div className="relative flex flex-col gap-1">
        ...
        <span
          className="font-serif font-normal text-[38px] leading-none tracking-[.3px] max-bp768:text-[32px]"
          style={{ color: toneFor(overall.score) }}
        >
          {labelFor(overall.score)}
        </span>
        <span
        ...
        >
          {overall.score > 0 ? "+" : ""}
          {overall.score} <span className="text-secondary">/ 100</span>
        </span>
        <span className="text-[12.5px] text-secondary mt-1.5 max-w-[240px]">
          {overall.note}
        </span>
      </div>
      <div className="relative flex flex-col gap-3.5">
        <div className="relative h-3 rounded-lg bg-[linear-gradient(90deg,var(--loss)_0%,#6c6786_50%,var(--gain)_100%)] opacity-[.92]">
          <div
            className="absolute top-[-5px] w-1 h-[22px] rounded-[3px] bg-primary shadow-[0_0_12px_var(--accent-glow),0_2px_6px_rgba(0,0,0,.5)] [transition:left_.7s_cubic-bezier(.2,.7,.2,1)]"
            style={{ left: `calc(${mark}% - 2px)` }}
          />
        </div>
        <div className="flex justify-between text-[10.5px] text-muted font-mono mt-[-4px]">
          <span>Bearish</span>
          <span>Neutral</span>
          <span>Bullish</span>
        </div>
        <div className="flex gap-[18px]">
          <span className="flex items-center gap-[7px] text-[12.5px] text-secondary">
            ...
            />
            <b className="text-primary font-semibold tabular-nums">
              {counts.bull}
            </b>{" "}
            Bullish
          </span>
...
const SUGGEST = [
  ...
  "Summarise my book in two lines",
...
function AskBox({ holdings }: { holdings: HoldingRow[] }) {
  ...
  const ask = async (text?: string) => {
    ...
    try {
      ...
      setAns(
        "The analysis engine is offline in this preview. Once connected, this answers questions about your book in plain language.",
      );
      setPhase("done");
    }
  };
  ...
  return (
    <div className="card flex flex-col gap-3 px-5 py-4.5 max-bp768:overflow-hidden max-bp480:p-3.5 max-bp380:p-3">
      ...
      <div className="flex gap-2 flex-wrap">
        {SUGGEST.map((s) => (
          <button
            className="font-ui text-xs text-secondary bg-elevated border border-subtle rounded-lg px-3 py-[7px] cursor-pointer transition-all duration-150 hover:text-gold hover:border-gold-soft"
            key={s}
            onClick={() => ask(s)}
          >
            {s}
      ...
      {phase === "thinking" && (
        <div className="text-[13.5px] leading-[1.6] text-primary border-l-2 border-gold-soft pl-4 py-1 [animation:fadeSlideUp_.4s_ease_both]">
          Reading your book
          <span className="inline-block w-[7px] h-[15px] bg-gold ml-0.5 align-[-2px] animate-blink" />
        </div>
      )}
...
function buildFallback(holdings: HoldingRow[]): AnalysisData {
  const items = holdings.map((h, i) => {
    ...
    const fb = FALLBACK_ITEMS[id] ?? {
      score: 0,
      summary: `${h.name} — no live analysis yet.`,
      drivers: [],
    };
    return {
      id,
...
async function runSentimentAI(holdings: HoldingRow[]): Promise<AnalysisData> {
  ...
  > = {};
  (json.items || []).forEach(
    (x: { id: string; score: number; summary: string; drivers: string[] }) => {
      byId[String(x.id)] = x;
    },
  );
  ...
  const items = assets.map((as) => {
    ...
    return {
      ...
      score: clamp((f as { score?: number }).score ?? 0),
      summary: (f as { summary?: string }).summary ?? "",
      drivers: ((f as { drivers?: string[] }).drivers ?? []).slice(0, 3),
    };
  });

  const ovScore =
    json.overall?.score != null
      ? clamp(json.overall.score)
      : mean(items.map((i) => i.score));
  return {
    items,
    overall: {
      score: ovScore,
      note: json.overall?.note ?? FALLBACK_OVERALL.note,
    },
    source: "ai",
  };
}
  ...
  return (
    <div className="flex flex-col gap-[18px] min-w-0 w-full">
      <div className="flex items-end justify-between gap-[18px] flex-wrap mb-0.5 animate-reveal">
        <div>
          <div className="text-[10.5px] uppercase tracking-[.14em] text-gold font-semibold">
            AI Analysis
          </div>
          <h2 className="font-serif font-normal text-[26px] mt-1.5 mb-1 tracking-[.2px]">
            Market sentiment
          </h2>
          <div className="text-[13px] text-secondary max-w-[440px]">
            An AI read on every holding — direction, conviction, and the drivers
            moving each position.
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      {data.source === "sample" && !aiRunning && (
        <div className="text-[11.5px] text-muted flex items-center gap-1.5">
          ...
          Showing curated sample data — set a real{" "}
          <code>ANTHROPIC_API_KEY</code> for a live read.
        </div>
      )}
    </div>
  );
