...
const STRAT: Record<string, { label: string; cls: string }> = {
  ...
  speculative: {
    label: "speculative",
    cls: "bg-[rgba(229,138,208,0.12)] text-fx-down",
  },
...
const ROW_UNSEL =
  "cursor-pointer border-l-2 border-l-transparent transition-[background] duration-150 hover:bg-elevated";
const ROW_SEL =
  "cursor-pointer border-l-2 border-l-gold bg-elevated transition-[background] duration-150 hover:bg-elevated";
const ROW_GROUP_HD =
  "cursor-pointer border-l-2 border-l-[rgba(212,163,78,.18)] bg-[rgba(212,163,78,.03)] transition-[background] duration-150 hover:bg-[rgba(212,163,78,.07)]";
const ROW_LOT_UNSEL =
  "cursor-pointer border-l-2 border-l-transparent bg-surface transition-[background] duration-150";
const ROW_LOT_SEL =
  "cursor-pointer border-l-2 border-l-gold bg-surface transition-[background] duration-150";

// .htable td (+ .r / .bold) variants
const CELL =
  "border-b border-subtle px-4 py-3.5 text-[13px] light:border-b-black/[.06]";
...
const LOT_CELL =
  "border-b border-subtle px-4 py-2.5 text-xs light:border-b-black/[.06]";
const LOT_CELL_FIRST =
  "border-b border-subtle py-2.5 pr-4 pl-11 text-xs light:border-b-black/[.06]";
  ...
  if (mode === "edit")
    return (
      <div className="flex flex-[0_0_300px] flex-col gap-[13px] rounded-[13px] border border-subtle bg-elevated p-4 light:border-black/10 max-bp768:flex-[0_0_280px] max-bp600:flex-[0_0_calc(100vw_-_56px)] animate-reveal">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Select
              ...
              onChange={(v) =>
                setEf(
                  "strategy",
                  Object.entries(STRAT_LABEL_EDIT).find(
                    ([, l]) => l === v,
                  )?.[0] ?? ef.strategy,
                )
              }
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              type="number"
              value={ef.units}
              onChange={(e) => setEf("units", e.target.value)}
            />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-semibold uppercase tracking-[.08em] text-muted">
            ...
            <input
              ...
              type="number"
              value={ef.buy_price}
              onChange={(e) => setEf("buy_price", e.target.value)}
            />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-semibold uppercase tracking-[.08em] text-muted">
            ...
            <input
              ...
              type="date"
              value={ef.buy_date}
              onChange={(e) => setEf("buy_date", e.target.value)}
            />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-semibold uppercase tracking-[.08em] text-muted">
        ...
        <button
          className="w-full cursor-pointer rounded-[9px] border-none bg-gold p-[9px] font-ui text-[13px] font-semibold text-[#15130c] transition-[filter] duration-150 hover:brightness-[1.08] disabled:cursor-not-allowed disabled:opacity-50"
          onClick={handleSave}
          disabled={saving}
        >
  ...
  return (
    <div className="flex flex-[0_0_300px] flex-col gap-[13px] rounded-[13px] border border-subtle bg-elevated p-4 light:border-black/10 max-bp768:flex-[0_0_280px] max-bp600:flex-[0_0_calc(100vw_-_56px)] animate-reveal">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="font-mono text-[10.5px] tracking-[.05em] text-muted">
              {h.ticker !== "—" ? h.ticker : h.assetType.toUpperCase()}
            </div>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-wrap items-baseline gap-2.5">
        <div
          className="font-mono text-[22px] font-semibold tracking-[-.01em]"
          style={{ color: total >= 0 ? "var(--gain)" : "var(--loss)" }}
        >
          {fmtSigned(total)}
        </div>
        <div
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center justify-between gap-3 text-[12.5px]">
          <span
            className="font-ui"
            style={{ color: h.assetGain >= 0 ? "var(--gain)" : "var(--loss)" }}
          >
          ...
          >
            {d.ccy !== "SGD"
              ? `${assetGainNative < 0 ? "−" : "+"}${NF(assetGainNative, 0)} ${d.ccy} = `
              : ""}
        ...
        <div className="flex items-center justify-between gap-3 text-[12.5px]">
          <span
            ...
            style={{
              color:
                h.fxGain >= 0 ? "var(--fx-positive)" : "var(--fx-negative)",
            }}
          >
        ...
        <div className="flex items-center justify-between gap-3 text-[12.5px] mt-[3px] border-t border-subtle pt-[7px]">
          ...
          <span className="font-mono text-primary text-[12.5px] font-semibold">
            {fmtSigned(total)}{" "}
            <span style={{ color: total >= 0 ? "var(--gain)" : "var(--loss)" }}>
              ({pct(h.totalPct)})
            </span>
          </span>
        </div>
      ...
      <div style={{ marginTop: 12 }}>
        <Dumbbell
          ...
          scale={
            Math.max(
              Math.abs(h.assetGain + h.fxGain),
              Math.abs(h.assetGain),
              1,
            ) * 1.2
          }
        />
      ...
      {mode === "confirm-delete" ? (
        <div className="flex flex-col gap-2.5 rounded-[10px] border border-[rgba(239,68,68,0.18)] bg-[rgba(239,68,68,0.05)] px-3.5 py-3">
          <span className="font-ui" style={{ fontSize: 12.5 }}>
            Delete <strong>{h.name}</strong> permanently?
          </span>
          <div className="flex gap-2">
            ...
            <button
              className="flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-[9px] border border-subtle bg-transparent p-[9px] font-ui text-[12.5px] font-medium transition-all duration-150 text-secondary hover:border-gold-soft hover:text-gold disabled:cursor-not-allowed disabled:opacity-50"
              onClick={() => setMode("view")}
            >
              Cancel
            </button>
...
export default function HoldingsPage() {
  ...
  const gKey = (g: GroupedHolding) =>
    g.ticker !== "—" ? g.ticker : g.lots[0].id;
  const isExpanded = (g: GroupedHolding) =>
    q !== "" || expandedTickers.has(gKey(g));
  ...
  function sortVal(
    ...
    switch (k) {
      ...
      case "assetGain":
        return item.assetGain;
      case "fxGain":
        return item.fxGain;
      case "totalPct":
        return item.totalPct;
    }
  }
  ...
  const handleCsvExport = () => {
    ...
    const rowData = holdings.map((h) => [
      ...
      h.assetType,
      h.broker,
      STRAT[h.strategy]?.label ?? h.strategy,
      h.units,
      h.currency,
      h.valueSGD.toFixed(2),
      h.assetGain.toFixed(2),
      h.fxGain.toFixed(2),
    ...
    <div className="flex w-full min-w-0 flex-col gap-[18px]">
      ...
      <div className="flex flex-wrap items-center gap-3 max-bp768:gap-2 max-bp600:flex-col max-bp600:items-stretch animate-reveal">
        <div className="flex flex-1 items-center gap-[9px] rounded-[10px] border border-subtle bg-surface px-[13px] py-[9px] text-muted min-w-[220px] max-bp600:w-full max-bp600:min-w-0">
          ...
          />
          <kbd className="rounded-[4px] border border-subtle px-1.5 py-px font-mono text-[10px] text-muted">
            /
          </kbd>
        </div>
        <div className="flex flex-wrap gap-1.5 max-bp768:gap-1">
          {TYPES.map((t) => (
            <button
              key={t}
              className={
                "cursor-pointer rounded-lg border px-[13px] py-[7px] font-ui text-xs transition-all duration-150 max-bp768:px-2.5 max-bp768:py-1.5 max-bp768:text-[11.5px] " +
                (typeFilter === t
        ...
        <button
          className="flex cursor-pointer items-center gap-[7px] rounded-[9px] border border-subtle bg-surface px-[13px] py-2 font-ui text-[12.5px] text-secondary transition-all duration-150 hover:border-gold-soft hover:text-gold disabled:cursor-not-allowed disabled:opacity-50"
      ...
      {/* table */}
      <div
        className="card p-0 overflow-x-auto overflow-y-hidden max-bp768:overflow-y-visible animate-reveal"
          ...
          <tbody>
            {groupView
              ? groups.flatMap((group) => {
                  ...
                  const gStrat = STRAT[group.lots[0].strategy] ?? {
                    label: group.lots[0].strategy,
                    cls: STRAT_GRAY,
                  };
                  ...
                  const groupRow = (
                    <tr
                      key={gk}
                      className={
                        isMulti ? ROW_GROUP_HD : gSel ? ROW_SEL : ROW_UNSEL
                      }
                      onClick={() =>
                        isMulti ? toggleGroup(gk) : toggle(group.lots[0])
                      }
                    >
                      <td className={CELL + (gSel ? SEL_SHADOW : "")}>
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="grid size-7 shrink-0 place-items-center rounded-[7px] border border-subtle bg-elevated text-gold">
                            <Icon name={group.icon as never} size={15} />
                          </span>
                          <span
                            className="font-ui text-[13px] overflow-hidden text-ellipsis whitespace-nowrap"
                            style={isMulti ? { fontWeight: 600 } : undefined}
                          >
                            {group.name}
                          </span>
                          {group.ticker !== "—" && (
                          ...
                          {isMulti && (
                            <span className="ml-1 inline-flex items-center rounded-[5px] bg-[rgba(212,163,78,.12)] px-[7px] py-0.5 font-ui text-[10.5px] font-semibold tracking-[.04em] text-gold">
                      ...
                      <td className={CELL}>
                        <span className={STRAT_BASE + " " + gStrat.cls}>
                          {gStrat.label}
                        </span>
                      </td>
                      ...
                      <td
                        ...
                        style={{
                          color: gTotal >= 0 ? "var(--gain)" : "var(--loss)",
                        }}
                      >
                        {pct(group.totalPct)}
                      </td>
                  ...
                  const lotRows = group.lots.map((h) => {
                    ...
                    return (
                      ...
                      >
                        <td
                          className={LOT_CELL_FIRST + (sel ? SEL_SHADOW : "")}
                        >
                        ...
                        <td className={LOT_CELL}>
                          <span className={STRAT_BASE + " " + lStrat.cls}>
                            {lStrat.label}
                          </span>
                        </td>
              ...
              : rows.map((h) => {
                  ...
                  return (
                    <tr
                      key={key(h)}
                      className={sel ? ROW_SEL : ROW_UNSEL}
                      onClick={() => toggle(h)}
                    >
                      <td className={CELL + (sel ? SEL_SHADOW : "")}>
                        <div className="flex items-center gap-2.5">
                          <span className="grid size-7 shrink-0 place-items-center rounded-[7px] border border-subtle bg-elevated text-gold">
                            <Icon name={h.icon as never} size={15} />
                          </span>
                          <span className="font-ui text-[13px] overflow-hidden text-ellipsis whitespace-nowrap">
                            {h.name}
                          </span>
                          <span className="font-mono text-[10.5px] tracking-[.05em] text-muted">
                            {h.ticker}
                      ...
                      <td className={CELL}>
                        <span className={STRAT_BASE + " " + strat.cls}>
                          {strat.label}
                        </span>
                      </td>
      ...
      <div
        className="flex flex-col gap-3 animate-reveal"
      ...
      >
        ...
        {compareCards.length > 0 ? (
          ...
          <div className="rounded-xl border border-dashed border-subtle bg-surface p-6 text-center text-[13px] font-ui text-secondary">
