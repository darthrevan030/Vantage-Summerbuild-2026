...
function CcyCard({ c, delay, sparkOverride }: { c: CurrencyCard; delay: string; sparkOverride?: number[] }) {
  ...
  return (
    <div className="card p-[18px] animate-reveal" style={{ animationDelay: delay }}>
      ...
      <div className="grid grid-cols-2 gap-3.5 mb-4">
        <div>
          <div className="font-ui text-secondary text-[11px] tracking-[.04em]">Exposure</div>
          <div className="font-mono text-[15px] font-semibold my-0.5">{fmtVal(c.exposure)}</div>
          <div className="font-ui text-secondary text-[11px] tracking-[.04em]">{c.exposurePct.toFixed(1)}% of book</div>
        </div>
        <div>
          <div className="font-ui text-secondary text-[11px] tracking-[.04em]">Avg / Current</div>
          <div className="font-mono text-[15px] font-semibold my-0.5">{rate(c.avg)} → {rate(c.cur)}</div>
          <div className="font-ui text-secondary text-[11px] tracking-[.04em]">SGD per {c.code}</div>
        </div>
      </div>
      <div className="flex items-end justify-between gap-3 pt-3.5 border-t border-subtle max-bp600:flex-col max-bp600:gap-2">
        <div>
          <div className="font-ui text-secondary text-[11px] tracking-[.04em]">FX Impact</div>
          <div className="font-mono text-[19px] font-semibold mt-0.5 max-bp480:text-[16px]" style={{ color: col }}>{fmtSigned(c.impact)}</div>
        </div>
        <div>
...
function Waterfall({ items, netImpact, netPct }: { items: WaterfallItem[]; netImpact: number; netPct: number }) {
  ...
  return (
    <div className="card px-5 py-4.5 max-bp768:overflow-hidden max-bp480:p-3.5 max-bp380:p-3 animate-reveal" style={{ animationDelay: ".28s" }}>
      <div className="flex items-baseline justify-between mb-4 max-bp600:flex-wrap max-bp600:gap-2 max-bp600:items-center">
        <span className="text-[13px] font-semibold text-primary tracking-[.01em]">Currency Contribution to Portfolio</span>
        <span className="font-ui text-secondary text-[11px]">{baseCurrency}</span>
      </div>
      <div className="flex flex-col gap-3">
        {items.map((it) => {
          ...
          return (
            <div className="grid grid-cols-[48px_1fr_110px] items-center gap-3.5 max-bp600:grid-cols-[40px_1fr_88px] max-bp600:gap-2 max-bp380:grid-cols-[36px_1fr_80px] max-bp380:gap-1.5" key={it.code}>
              <span className="font-mono text-[13px] font-semibold text-secondary">{it.code}</span>
        ...
        <div className="flex items-center justify-between mt-1.5 pt-3.5 border-t border-subtle text-[14px]">
          ...
          <span className="font-mono font-semibold text-[15px]" style={{ color: netImpact >= 0 ? "var(--fx-positive)" : "var(--fx-negative)" }}>
            {fmtSigned(netImpact)}{" "}
    ...
    return (
      <div className="flex flex-col gap-[18px] min-w-0 w-full max-bp768:box-border">
        <div className="card px-5 py-4.5 max-bp768:overflow-hidden max-bp480:p-3.5 max-bp380:p-3">
          <div className="flex items-baseline justify-between mb-4 max-bp600:flex-wrap max-bp600:gap-2 max-bp600:items-center">
            <span className="text-[13px] font-semibold text-primary tracking-[.01em]">FX Lab</span>
            <span className="font-ui text-secondary text-[11px]">No foreign-currency holdings found</span>
          </div>
  ...
  return (
    <div className="flex flex-col gap-[18px] min-w-0 w-full max-bp768:box-border">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-5 bg-surface border border-subtle rounded-[16px] px-8 py-7 relative animate-reveal max-bp600:grid-cols-1 max-bp600:gap-3 max-bp600:px-5 max-bp600:py-[22px] max-bp600:text-center">
        <div>
          <div className="font-ui text-secondary text-[11px] uppercase tracking-[.1em] mb-2">Actual Portfolio</div>
          <div className="font-serif text-[40px] leading-none tracking-[-.01em] max-bp900:text-[32px] max-bp600:text-[26px] max-bp480:text-[22px]">{fmtVal(actual)}</div>
        </div>
        <div className="grid place-items-center max-bp600:hidden">
          <div className="w-[42px] h-[42px] rounded-full border border-subtle grid place-items-center text-muted text-[13px] italic font-ui">vs</div>
        </div>
        <div className="text-right max-bp600:text-center">
          <div className="font-ui text-secondary text-[11px] uppercase tracking-[.1em] mb-2">FX-Neutral Portfolio</div>
          <div className="font-serif text-[40px] leading-none tracking-[-.01em] text-muted max-bp900:text-[32px] max-bp600:text-[26px] max-bp480:text-[22px]">{fmtVal(neutral)}</div>
        </div>
        <div className="col-span-full flex items-center justify-center gap-2.5 mt-[18px] pt-[18px] border-t border-subtle text-[14px] text-secondary max-bp600:flex-col max-bp600:gap-1.5 max-bp600:text-center">
          ...
          <span className="font-ui">
            ...
            <b className="font-mono font-semibold" style={{ color: verdictPositive ? "var(--fx-positive)" : "var(--fx-negative)" }}>
              {fmtVal(Math.abs(hero.fxImpact))}
            </b>{" "}
            <span className="font-mono" style={{ color: verdictPositive ? "var(--fx-positive)" : "var(--fx-negative)" }}>
              ({pct(hero.fxPct)})
            </span>{" "}
            to your returns
          </span>
